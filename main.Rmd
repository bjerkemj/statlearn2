...
One interesting thing to note here is that **maxHR** has been left out, which is probably due to it's negative correlation with **age**. This model prediction of `r round(100*glm.best.accuracy, 2)`% which is slightly better than the first model. But looking at the confusion matrix below we see that one more person been classified as false negative. Due to lack of time we would probably run cross-validation on these to models to see if there is any difference between them, but the accuracy and confusion matrix seems to suggest basicly no difference. In that case the latter model is preferred because it is smaller. One advantege of this is that it is easier to explain and understand the influence of each predictor.
