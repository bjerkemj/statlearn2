...
diseases (CVDs) which in many cases leads to heart failure, which stands for 
...
```
It's important to realize that this distribution is not reflected in real life, and therefore a model's performance on new data may be poor, as the model has learned the patterns specific to the training data. It's important to be aware of this, as it may lead to inference bias. However, as we see in the Cholesterol vs. Heart Disease, most of the zero-cholesterol observations are also heart disease observations, so removing these will somewhat adjust the unbalanced distribution. Have a look below:
