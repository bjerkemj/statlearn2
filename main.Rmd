...
```
From the data analysis, we concluded that are some false cholesterol observations. There are several approaches to deal with missing data. We considered filling the values using kNN, but since removing the observations adjusted the imbalanced **heart disease** vs not. distribution, we decided to drop them.
